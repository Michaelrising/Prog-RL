...
class readData():
	...
	#获取给定模式的持续时间，输入模式向量np.array，shape=[1,n]，返回对应的持续时间向量np.array，shape=[1,n]
        duration = []
        for i in range(modesequece.shape[1]):
	...
	#根据给定的活动列表以及该活动选择的模式，构建各个mode之间的时间限制矩阵
    #构建sslimit矩阵
