...
class readData():
    ...
    # mode_using_renewable_resource:当前模式队列下，每个模式需要消耗的可更新资源,类型为np.array,shape=[n,K0]
        ...
        # print(start_timesequence,end_timesequence)
    ...
    #随机生成时间序列，为0-1之间的小数，保留三位
        time_chromosome = np.random.randint(0, 100, [1, self.n])/1000
    ...
    #根据最短距离矩阵，进行时间序列解码，设置最后一个序列的最晚开始时间为s(n+1)+d_ba
        # print(time_chromosome)
        INF = - (np.max(M) + d_ba)
    ...
    #读取.sch中的数据并转化为相对应的数据结构
        ...
        if(len(filename) > 0):    
            ...
            # print(self.strong_connect_components)
            # print(graph_data)
